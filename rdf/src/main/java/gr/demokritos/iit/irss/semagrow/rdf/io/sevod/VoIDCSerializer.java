...
import gr.demokritos.iit.irss.semagrow.rdf.RDFCircle;
...
public class VoIDCSerializer {
    ...
    public VoIDCSerializer(String format, String outputPath) {
    ...
    public void serialize(STHolesCircleHistogram<RDFCircle> histogram) throws RDFHandlerException {
     ...
     */
    private void serializeBucket(STHolesBucket<RDFCircle> bucket, Resource bucketResource, int num, boolean isRoot) {
        ...
        if (bucket.getBox().getSubjectRange().isInfinite())
            ...
            // Subject Range
            //for (String s : bucket.getBox().getSubjectRange().getCenter()) {
            String s = bucket.getBox().getSubjectRange().getCenter();
        ...
        int count = 0;
        for (STHolesBucket<RDFCircle> b : bucket.getChildren()) {
            count++;
