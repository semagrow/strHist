...
/**
 * Created by katerina on 10/12/2015.
 */
public class CardinalityCircleEstimatorImpl implements CardinalityEstimator {
    private Histogram<RDFCircle> histogram;

    public CardinalityCircleEstimatorImpl(Histogram<RDFCircle> histogram) {
    ...
    private RDFCircle toRectangle(StatementPattern pattern, BindingSet bindings) {
        ...
        if (!list.isEmpty())
            subjectRange = new RDFStrRange(list);
        else
            subjectRange = new RDFStrRange();*/
        if (sVal == null)
            subjectRange = new RDFStrRange();
        else
            subjectRange = new RDFStrRange(sVal.stringValue());
