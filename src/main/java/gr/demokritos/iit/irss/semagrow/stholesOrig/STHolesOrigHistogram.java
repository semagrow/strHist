 ...
 */
public class STHolesOrigHistogram<R extends RectangleWithVolume<R>> implements STHistogram<R> {
    ...
    public STHolesOrigHistogram(Iterable<QueryRecord<R>> workload) {
        this();
        refine(workload);
    }
     ...
     */
    private STHolesOrigBucket<R> shrink(STHolesOrigBucket<R> bucket, R rect, QueryRecord<R> queryRecord) {
    ...
    private long countMatchingTuples(R rectangle, QueryRecord<R> queryRecord) {
        ...
        return qr.getCardinality(rectangle).getFrequency();
    }
